    ...
    <div className="space-y-5">
      ...
      <section aria-label="Overviews" className="grid gap-4 lg:grid-cols-3">
        ...
        <CommunicationHub />
            ...
            <section className="grid gap-4 lg:grid-cols-2"><StatusStrip title="Globe Clients" clients={globeClients} href="/globe" /><StatusStrip title="SME Clients" clients={smeClients} href="/sme" /></section>
      <section className="grid gap-4 lg:grid-cols-5"><AiSupport clients={clients} bugReport={bugReport} rscReport={rscReport} contractReport={contractReport} /><div className="min-h-44 rounded-xl border border-line bg-surface p-5 lg:col-span-2"><h2 className="text-sm font-bold">Quick Links</h2><div className="mt-3 space-y-3 text-xs">{[{ label: "YBO Training Videos", href: "/training/videos" }, { label: "Training Materials", href: "/training/materials" }, { label: "Client Contracts (SME & Globe)", href: "/training/contracts" }, { label: "Notion (Bugs / Projects)", href: null }, { label: "Pumble Channels", href: null }].map((link) => link.href ? <Link href={link.href} className="flex items-center justify-between hover:text-blue-600" key={link.label}><span className="flex items-center gap-2"><MessageCircle size={15} className="text-blue-500" />{link.label}</span><ArrowRight size={13} className="text-muted" /></Link> : <p className="flex items-center justify-between text-muted" key={link.label}><span className="flex items-center gap-2"><MessageCircle size={15} className="text-blue-300" />{link.label}</span><span className="text-[10px]">Coming soon</span></p>)}</div></div></section>
    </div>
  );
}

function StatusStrip({ title, clients, href }: { title: string; clients: Client[]; href: string }) { const counts = clients.reduce((result, client) => { result[currentClientStatus(client)] += 1; return result; }, { ACTIVE: 0, EXPIRING_SOON: 0, EXPIRED: 0 }); return <Link href={href} className="block rounded-xl border border-line bg-surface p-4 transition hover:border-blue-300 hover:shadow-sm"><div className="flex justify-between text-sm font-bold"><span><Globe2 size={14} className="mr-2 inline text-emerald-500" />{title}</span><span>{clients.length} <small className="font-normal text-muted">Total</small></span></div><div className="mt-3 flex gap-2"><span className="rounded-lg bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-500">{counts.ACTIVE}<small className="ml-1 block text-[9px] font-normal">Active</small></span><span className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-bold text-amber-500">{counts.EXPIRING_SOON}<small className="ml-1 block text-[9px] font-normal">Expiring Soon (30 days)</small></span><span className="rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-500">{counts.EXPIRED}<small className="ml-1 block text-[9px] font-normal">Expired</small></span></div></Link> }
